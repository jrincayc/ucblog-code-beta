...
typedef enum NodeTypes  {
  ...
  NT_COLON =          000002,
  NT_CASEOBJ =        000001,
  PNIL =    NT_EMPTY|NT_AGGR|NT_LIST,
  CONS =    NT_AGGR|NT_LIST,
  STRING =  NT_WORD|NT_STRING,
  INT =     NT_WORD|NT_NUMBER,
  FLOATT =  NT_WORD|NT_NUMBER|NT_FLOAT,
  PRIM =    NT_PRIM,
  MACRO =   NT_PRIM|NT_MACRO,
  TAILFORM = NT_PRIM|NT_TAILFORM,
  CASEOBJ = NT_WORD|NT_CASEOBJ,
  INFIX =   NT_PRIM|NT_INFIX,
  TREE =    NT_AGGR|NT_LIST|NT_TREE,
  RUN_PARSE = NT_AGGR|NT_LIST|NT_RUNP,
  QUOTE =   NT_WORD|NT_PUNCT,
  COLON	=   NT_WORD|NT_PUNCT|NT_COLON,
  BACKSLASH_STRING = NT_WORD|NT_STRING|NT_BACKSL,
  VBAR_STRING = NT_WORD|NT_STRING|NT_BACKSL|NT_VBAR,
  ARRAY	=   NT_AGGR|NT_ARRAY,
  LINE =    NT_LINE|NT_LIST|NT_AGGR,
  CONT =    NT_CONT|NT_LIST,
  STACK =   NT_STACK|NT_LIST,
  LOCALSAVE = CONS|NT_LOCAL,
} NODETYPES;
...
// in a shift by 1 when compiled with OBJECTS
    ...
    /* below this point aren't actually error codes, just messages */
		...
		NO_FILE, NO_FIONREAD, NO_WXWIDGETS, MEM_LOW, CANT_OPEN, ALREADY_OPEN,
